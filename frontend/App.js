...
// (useState, useContext, createContext, useEffect, useRef) are automatically available in the global scope.
const { useState, useContext, createContext, useEffect, useRef } = React;

/* ---------------------------------------------
   MOCK PRODUCT DATA (static for the POC)
---------------------------------------------- */
const productData = [
  {
    id: 'prod_001',
    ...
    price: 1299.99,
    description:
      'A sleek, powerful laptop with a next-generation quantum processor. Perfect for developers and creators.',
  ...
  },
  {
    ...
    price: 159.5,
    description:
      'A split ergonomic keyboard with satisfying mechanical switches. Type for hours in comfort.',
  ...
  },
  {
    ...
    price: 799.0,
    description:
      'Immerse yourself in stunning detail with this 34-inch ultra-wide 4K monitor.',
    category: 'Monitors',
    stock: 22,
  ...
  },
  {
    ...
    price: 89.99,
    description:
      'Keeps your drink at the perfect temperature, controlled by an app. The future of beverage consumption.',
    category: 'Gadgets',
    stock: 120,
  ...
  },
  {
    ...
    price: 349.99,
    description:
      'Zone in on your work or music with industry-leading noise cancellation and premium sound.',
    category: 'Audio',
    stock: 50,
  ...
  },
  {
    ...
    price: 129.99,
    description:
      'Blazing fast, pocket-sized storage for your files, games, and projects.',
    category: 'Storage',
    stock: 75,
          ...
          >
            ...
            <div className="p-6 flex-grow flex flex-col">
              ...
              <div className="mt-6 flex justify-between items-center">
                ...
                <button
                  onClick={() =>
                    setPage({ name: 'productDetail', productId: product.id })
                  }
      ...
      {cart.length === 0 ? (
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mt-6 text-right">
            ...
            <button className="mt-4 bg-green-500 text-white px-8 py-3 rounded-lg">
              Proceed to Checkout
            </button>
          </div>
        </div>
...
/* ---------------------------------------------
   CHAT WIDGET
---------------------------------------------- */
function ChatWidget({ onAction, uiContext = { name: null, productId: null } }) {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    { sender: 'bot', text: 'Hello! How can I help you shop today?' },
  ]);
    ...
    try {
      const response = await fetch('http://localhost:8001/query', {
        ...
        body: JSON.stringify({
          ...
          ui_context: {
            name: uiContext?.name ?? null,
            productId: uiContext?.productId ?? null,
          },
        }),
      });
      ...
      const botMessage = { sender: 'bot', text: action.reply || "Okay, I've handled that for you." };
...
function EcommerceApp() {
  ...
  // Executes server-returned actions (RAG or finetuned model),
  // accepting either {id} or {productId} for compatibility.
  const executeAction = (action) => {
    console.log('Executing action:', action);
    if (!action || !action.action || !action.payload) return;
    ...
    switch (action.action) {
      ...
      case 'addToCart': {
        ...
        const product = productData.find((p) => p.id === targetId);
        if (product) addToCart(product);
        else console.error('Product not found for addToCart:', targetId);
        break;
      }
      case 'removeFromCart': {
        const targetId = action.payload.id || action.payload.productId;
        if (targetId) removeFromCart(targetId);
        else console.error('No product id for removeFromCart payload');
        break;
      }
      default:
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      ...
      <ChatWidget onAction={executeAction} uiContext={page} />
    </div>
  );
}
